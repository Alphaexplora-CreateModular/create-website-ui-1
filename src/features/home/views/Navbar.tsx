...
export function Navbar({ activeSection, isHeaderScrolled }: NavbarProps) {
  ...
  const isNightMode = isInHeader && isHeaderScrolled;

  useEffect(() => {
    setIsToggled(isNightMode);
  }, [isNightMode]);

  const toggleTrackClass = isToggled
    ? "bg-[#4C3E39] shadow-black/30"
    : "bg-[#FFF2E2] shadow-black/30";
  const toggleKnobClass = isToggled ? "bg-[#E6E0DC]" : "bg-[#4C3E39]";
  const logoSrc = isToggled ? "/logo-brown.svg" : "/logo.svg";
